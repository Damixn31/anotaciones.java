package org.olmedo.anotaciones.ejemplo.procesador;

import org.olmedo.anotaciones.ejemplo.Init;
import org.olmedo.anotaciones.ejemplo.JsonAtributo;
import org.olmedo.anotaciones.ejemplo.procesador.exception.JsonSerializadorException;
...
public class JsonSerializador {
                ...
                .forEach(m -> {
                    ...
                    try {
                       ...
                       throw new JsonSerializadorException(
                               "Error al serializar, no se puede inizializar el objeto!"
                       + e.getMessage());
                    }
                });
    }
    public static String convertirJson(Object object){
        ...
        inicializarObjeto(object); //aca inicializamos el metodo
        Field[] atributos = object.getClass().getDeclaredFields();
        return Arrays.stream(atributos)
                .filter(f -> f.isAnnotationPresent(JsonAtributo.class))
                .map(f -> {
                    f.setAccessible(true); // si el atributo es privado se puede acceder
                    ...
                    try {
                        ...
                        valor instanceof String){
                            ...
                            nuevoValor = Arrays.stream(nuevoValor.split(" "))
                                    ...
                                    .collect(Collectors.joining(" "));/*String.valueOf(nuevoValor.charAt(0)).toUpperCase() +
